...
import { Gender, Hari, JabatanDosen, KRSStatus, Pendidikan, Prisma, Role, Semester, Status, StatusKRS } from "@prisma/client";
import { prisma } from "../src/lib/prisma";
import { hashPassword } from "../src/lib/bycript";
import { programs } from "./seed-data/catalog";
...
const statuses = [StatusKRS.DISETUJUI, StatusKRS.DISETUJUI, StatusKRS.DISETUJUI, StatusKRS.DIAJUKAN, StatusKRS.DITOLAK, StatusKRS.DRAFT];
const days = [Hari.SENIN, Hari.SELASA, Hari.RABU];

// Hanya dijalankan jika operator secara eksplisit memberikan --reset.
    ...
    for (const [p, definition] of programs.entries()) {
      ...
      for (const [periodIndex, year] of years.entries()) {
        ...
        for (const [i, nama] of names.entries()) {
          ...
          await tx.jadwal.create({ data: { kelasMataKuliahId: assignment.id, tahunAkademikId: year.id, ruanganId: room.id, hari: days[Math.floor(i / 2)], hariUrutan: Math.floor(i / 2) + 1, jamMulai: i % 2 === 0 ? "08:00" : "10:45", jamSelesai: i % 2 === 0 ? "10:30" : "13:15" } });
        }
