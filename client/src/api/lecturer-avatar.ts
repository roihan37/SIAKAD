...
export async function uploadLecturerAvatar(id: string, photo: string): Promise<string> {
  const isCropSource = photo.startsWith("blob:") || /^data:image\/(png|jpeg|webp);base64,/.test(photo)
  if (!isCropSource) throw new Error("Format foto hasil crop tidak valid.")
  const imageResponse = await fetch(photo)
  if (!imageResponse.ok) throw new Error("Gagal memproses foto hasil crop.")
  const blob = await imageResponse.blob()
  if (!["image/png", "image/jpeg", "image/webp"].includes(blob.type)) throw new Error("Foto hasil crop harus berupa PNG, JPEG, atau WebP.")
